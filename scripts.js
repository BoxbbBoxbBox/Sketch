...
button.addEventListener("click", function () {
  const size = Number(
    prompt(
      "Choose a positive integer up to 100. This will be the length of a row."
    )
  );
  if (Number.isInteger(size) && size < 101 && size > 0) {
    ...
    function sizeSquares() {
      for (let i = 0; i < userCalculated; i++) {
        allSquares[i].style.flexBasis =
          (960 / userCalculated) * userInput + "px";
        allSquares[i].style.height = (960 / userCalculated) * userInput + "px";
        allSquares[i].textContent += "a";
      }
    }
    sizeSquares();
    for (let i = 0; i < userCalculated; i++) {
      allSquares[i].onmouseenter = function () {
        mouseEnter();
      };
      function mouseEnter() {
